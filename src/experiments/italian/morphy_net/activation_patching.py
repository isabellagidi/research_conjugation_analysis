...
italian_conjugations = italian_conjugations[:100]
...
# Build prompts
p1, a1, e1 = build_conjugation_prompts(tokenizer, italian_first_sing, italian_conjugations)
p2, a2, e2 = build_conjugation_prompts(tokenizer, italian_second_sing, italian_conjugations)
...
for (inf_len, conj_len), group in top_5:
    ...
    save_heatmap(
        ...
        title=f"resid_pre Activation Patching Italian (inf={inf_len}, conj={conj_len})",
        filename=f"resid_pre_activation_patching_inf{inf_len}_conj{conj_len}_italian.png"
    )
...
attn_head_out_all_pos_patch_results_italian = patching.get_act_patch_attn_head_out_all_pos(
    tl_model, corrupted_tokens, clean_cache, patching_metric=conjugation_metric
)

save_heatmap(
    data=attn_head_out_all_pos_patch_results_italian,
    x_labels=[f"H{h}" for h in range(tl_model.cfg.n_heads)],
    y_labels=[f"L{l}" for l in range(tl_model.cfg.n_layers)],
    title="attn_head_out Activation Patching (All Positions) Italian",
    #filename="attn_head_out_all_pos_first2second_italian.png",
    filename="attn_head_out_all_pos_second2first_italian.png"
)

#torch.save(attn_head_out_all_pos_patch_results_italian, "attn_head_out_all_pos_patch_results_italian.pt")
torch.save(attn_head_out_all_pos_patch_results_italian, "attn_head_out_all_pos_patch_results_italian_second2first_italian.pt")
