...
# SWITCHED Second → First: now group "second person" as the clean set
...
for (inf_len, conj_len), group in top_5:
    ...
    save_heatmap(
        ...
        title=f"resid_pre Activation Patching Spanish (inf={inf_len}, conj={conj_len})",
        filename=f"resid_pre_activation_patching_inf{inf_len}_conj{conj_len}_spanish.png"
    )
...
attn_head_out_all_pos_patch_results_spanish = patching.get_act_patch_attn_head_out_all_pos(
    tl_model, corrupted_tokens, clean_cache, patching_metric=conjugation_metric
)

save_heatmap(
    data=attn_head_out_all_pos_patch_results_spanish,
    x_labels=[f"H{h}" for h in range(tl_model.cfg.n_heads)],
    y_labels=[f"L{l}" for l in range(tl_model.cfg.n_layers)],
    title="attn_head_out Activation Patching Spanish (All Positions)",
    #filename="attn_head_out_all_pos_first2second_spanish.png",
    filename="attn_head_out_all_pos_second2first_spanish.png"
)

#torch.save(attn_head_out_all_pos_patch_results_spanish, "attn_head_out_all_pos_patch_results_spanish_first2second.pt")
torch.save(attn_head_out_all_pos_patch_results_spanish, "attn_head_out_all_pos_patch_results_spanish_second2first.pt")
