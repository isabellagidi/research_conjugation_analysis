...
# -------------------- config --------------------
RESULTS_BASE = Path("results")                           # where results live
OUTPUT_BASE  = Path("RSA_model_comparisons_reduced_double")     # reduced outputs folder
...
# Conjugation pairs (directory names)
PAIRS = ["1pl-2pl", "1sg-1pl", "1sg-2sg", "1sg-3sg"]

# Toggle: if True, force a global language intersection across *all* models (per pair+direction).
...
def pair_to_directions(pair: str) -> list[str]:
    """'a-b' -> ['ato b', 'bto a'] as ['ato b','bto a'] with 'to' (no spaces)."""
    a, b = [x.strip() for x in pair.split("-")]
    return [f"{a}to{b}", f"{b}to{a}"]

def list_available_languages(results_root: Path, model_hf: str, pair: str, direction_tag: str):
    """
    Return language folder names (full names) that:
      1) are in the reduced set, and
      2) contain attn_head_out_all_pos_patch_results_<direction_tag>_<lang>.pt
    """
    ...
    for d in sorted(p.name for p in pair_dir.iterdir() if p.is_dir()):
        if d not in REDUCED_LANG_FULL:
            continue
        expected = pair_dir / d / f"attn_head_out_all_pos_patch_results_{direction_tag}_{d}.pt"
        if expected.exists():
            langs.append(d)
    return langs

def load_language_vectors(results_root: Path, model_hf: str, pair: str, direction_tag: str, langs: list[str]) -> torch.Tensor:
    """
    For (model, pair, direction), load flattened patch tensors per language into a matrix (L, D).
    """
    family, name = model_hf.split("/", 1)
    pair_dir = results_root / family / name / pair
    ...
    vecs = []
    for lang in langs:
        pt_path = pair_dir / lang / f"attn_head_out_all_pos_patch_results_{direction_tag}_{lang}.pt"
...
def second_order_similarity(model_rsa_a, langs_a, model_rsa_b, langs_b, use_global=None, global_langs=None):
    """
    Pearson correlation between upper-tri vectors of two RSA matrices,
    aligned on either pairwise intersection or provided global_langs.
    """
...
def main():
    ...
    for pair in PAIRS:
        directions = pair_to_directions(pair)  # e.g., ['1sgto2sg', '2sgto1sg']
        for direction_tag in directions:
            print(f"\n=== Second-order RSA (reduced langs) for pair: {pair} | dir: {direction_tag} ===")
            model_langs = {}
            model_rsas  = {}
            kept_models = []
            ...
            for m in MODELS:
                langs = list_available_languages(RESULTS_BASE, m, pair, direction_tag)
                if len(langs) < 2:
                    print(f"⚠️  {m} has <2 reduced languages for {pair} [{direction_tag}]; skipping.")
                    continue
                V = load_language_vectors(RESULTS_BASE, m, pair, direction_tag, langs)
                rsa = cosine_rsa_from_vectors(V)  # (L, L)
                model_langs[m] = langs
                model_rsas[m]  = rsa
                kept_models.append(m)
                print(f"✅ {m}: {len(langs)} reduced languages for {pair} [{direction_tag}]: {langs}")

            if len(kept_models) < 2:
                print(f"❌ Not enough models with reduced-language data for {pair} [{direction_tag}]; skipping.")
                continue

            # 2) optional: global intersection (per pair+direction)
            global_langs = None
            if STRICT_GLOBAL_INTERSECTION:
                sets = [set(model_langs[m]) for m in kept_models]
                inter = set.intersection(*sets) if sets else set()
                global_langs = sorted(inter)
                print(f"Global intersection for {pair} [{direction_tag}]: {len(global_langs)} languages — {global_langs}")

            # 3) model×model similarities
            M = len(kept_models)
            ...
            # 4) save (per pair×direction)
            out_dir = OUTPUT_BASE / pair / direction_tag
            ensure_dir(out_dir)
            labels = [pretty_axis_label(m) for m in kept_models]

            base = f"model_similarity_{pair}_{direction_tag}"
            torch.save(torch.tensor(sim), out_dir / f"{base}.pt")
            np.save(out_dir / f"{base}.npy", sim)
            plot_heatmap(sim, labels,
                         f"Second-order RSA (reduced langs) — {pair} • {direction_tag}",
                         out_dir / f"{base}.png")
