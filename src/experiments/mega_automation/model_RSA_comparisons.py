...
OUTPUT_BASE  = Path("RSA_model_comparisons")    # new folder for outputs
...
def list_available_languages(results_root: Path, model_hf: str, pair: str):
    """Return list of language folder names that actually contain the expected .pt file."""
...
def second_order_similarity(model_rsa_a, langs_a, model_rsa_b, langs_b, use_global=None, global_langs=None):
    ...
    if len(common) < 2:
        return np.nan  # not enough overlap to form upper-tri
...
def plot_heatmap(arr: np.ndarray, labels, title, out_png):
    n = len(labels)
    # bigger figure scaling with number of models
...
def main():
    ...
    for pair in PAIRS:
        print(f"\n=== Building second-order RSA for pair: {pair} ===")
        # 1) discover languages per model, and build each model's RSA
        model_langs = {}
        model_rsas  = {}
        kept_models = []
        ...
        for m in MODELS:
            ...
            if len(langs) < 2:
                print(f"⚠️  {m} has <2 languages for {pair}; skipping.")
                continue
            ...
            print(f"✅ {m}: {len(langs)} languages for {pair}")

        if len(kept_models) < 2:
            print(f"❌ Not enough models with data for {pair}; skipping.")
            continue
        ...
        if STRICT_GLOBAL_INTERSECTION:
            ...
            print(f"Global intersection for {pair}: {len(global_langs)} languages")
        ...
        ensure_dir(out_dir)
        # axis labels: only the part after the slash
        ...
        plot_heatmap(sim, labels, f"Second-order RSA (cosine-RSA) — {pair}", out_dir / f"model_similarity_{pair}.png")
