...
RSA_ROOT     = os.path.join(BASEDIR, "RSA_plots_reduced_double")    # <<< save reduced, both-directions here
...
# === Conjugation pairs (folder uses hyphen) ===
# We'll evaluate BOTH directions per pair (e.g., 1sgto2sg and 2sgto1sg).
PAIRS_HYPHEN = ["1pl-2pl", "1sg-1pl", "1sg-2sg", "1sg-3sg"]

def pair_to_directions(pair_hyphen: str):
    """Convert 'a-b' -> ['ato b', 'bto a'] (strip stray spaces)."""
...
def compute_rsa(heatmaps: dict[str, torch.Tensor]):
    """
    Return (labels, None, None, cos).
    Only cosine similarity is computed; MSE/Pearson are omitted.
    """
...
def main():
    for friendly_model in MODELS:
        ...
        for pair_hyphen in PAIRS_HYPHEN:
            ...
            directions = pair_to_directions(pair_hyphen)  # e.g., ["1sgto2sg", "2sgto1sg"]
            ...
            for direction_tag in directions:
                ...
                # compute cosine RSA for this direction
                labels, _, _, cos = compute_rsa(heatmaps)
                if cos is None:
                    print(f"↪️  Not enough languages for RSA: model={friendly_model}, pair={pair_hyphen}, dir={direction_tag}")
                    continue
                ...
                # save PT bundle for this direction
                bundle = {
                    "languages": labels,
                    "cos": cos,
                    "model_family": family,
                ...
                # save plot for this direction
                plot_rsa(
                    cos, labels,
                    f"{friendly_model} • {pair_hyphen} • {direction_tag} • RSA (Cosine)",
