...
RSA_ROOT     = os.path.join(BASEDIR, "RSA_plots")   # where to save RSA outputs
...
def main():
    for friendly_model in MODELS:
        ...
        for pair_hyphen in PAIRS_HYPHEN:
            ...
            # load per-language patch heatmaps
            ...
            # plot_rsa(mse,  labels, f"{friendly_model} • {pair_hyphen} • RSA (MSE)",
            #          os.path.join(outdir, f"rsa_mse_{pair_fname}.png"), cmap="viridis")
            # plot_rsa(corr, labels, f"{friendly_model} • {pair_hyphen} • RSA (Pearson)",
            #          os.path.join(outdir, f"rsa_corr_{pair_fname}.png"), cmap="coolwarm", center=0.0)
            plot_rsa(cos,  labels, f"{friendly_model} • {pair_hyphen} • RSA (Cosine)",
