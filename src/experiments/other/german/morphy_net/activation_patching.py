...
german_conjugations = german_conjugations[:100]
...
# Build prompts
p1, a1, e1 = build_conjugation_prompts(tokenizer, german_first_sing, german_conjugations)
p2, a2, e2 = build_conjugation_prompts(tokenizer, german_second_sing, german_conjugations)
...
for (inf_len, conj_len), group in top_5:
    ...
    save_heatmap(
        ...
        title=f"resid_pre Activation Patching german (inf={inf_len}, conj={conj_len})",
        filename=f"resid_pre_activation_patching_inf{inf_len}_conj{conj_len}_german.png"
    )
...
attn_head_out_all_pos_patch_results_german = patching.get_act_patch_attn_head_out_all_pos(
    tl_model, corrupted_tokens, clean_cache, patching_metric=conjugation_metric
)

save_heatmap(
    data=attn_head_out_all_pos_patch_results_german,
    x_labels=[f"H{h}" for h in range(tl_model.cfg.n_heads)],
    y_labels=[f"L{l}" for l in range(tl_model.cfg.n_layers)],
    #title="attn_head_out Activation Patching (All Positions) german first2second",
    title="attn_head_out Activation Patching (All Positions) german second2first",
    #filename="attn_head_out_all_pos_first2second_german.png",
    filename="attn_head_out_all_pos_first2second_german_second2first.png"
)

#torch.save(attn_head_out_all_pos_patch_results_german, "attn_head_out_all_pos_patch_results_german_first2second.pt")
torch.save(attn_head_out_all_pos_patch_results_german, "attn_head_out_all_pos_patch_results_german_second2first.pt")