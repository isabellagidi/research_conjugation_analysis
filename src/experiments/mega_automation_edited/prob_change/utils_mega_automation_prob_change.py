...
def compute_gold_logprobs(tl_model, prompts, answer_ids, device):
    """
    Return per-example log p(gold) at the final position for prompts.
    Uses the same tokenization path as your patching (TransformerLens).
    """
    toks = tl_model.to_tokens(prompts).to(device)
    logits = tl_model(toks)                # (B, T, V)
    last = logits[:, -1, :]               # (B, V)
    ans = torch.tensor(answer_ids, device=last.device)
    logps = last.log_softmax(dim=-1).gather(1, ans.unsqueeze(1)).squeeze(1)
    return logps.detach().cpu().tolist()

def make_bins_from_probs(probs):
    """
    probs: list[float] of probabilities (NOT log-probs).
    Returns dict(bin_name -> list of indices) using:
      lowest: p < 0.2
      low:    0.2 <= p < 0.5
      med:    0.5 <= p < 0.8
      high:   p >= 0.8
    """
    bins = {"lowest": [], "low": [], "med": [], "high": []}
    for i, p in enumerate(probs):
        if p < 0.2:
            bins["lowest"].append(i)
        elif p < 0.5:
            bins["low"].append(i)
        elif p < 0.8:
            bins["med"].append(i)
...
def align_corrupt_indices_by_lemma(clean_indices, clean_entries, corrupt_entries):
    """
    Given indices into the CLEAN set, return corresponding indices into the
    CORRUPT set by matching lemma (entry[0]). Keeps only aligned pairs.
    """
    lemma2idx_corrupt = {entry[0]: i for i, entry in enumerate(corrupt_entries)}
    out_clean, out_corrupt = [], []
    for i in clean_indices:
        lemma = clean_entries[i][0]
        j = lemma2idx_corrupt.get(lemma, None)
        if j is not None:
            out_clean.append(i)
            out_corrupt.append(j)
    return out_clean, out_corrupt
...
def accuracy_filter(
    ...
    NEW: also returns gold-token log-probabilities for correct/incorrect items.
    """
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    tokenizer.pad_token = tokenizer.eos_token
    model = AutoModelForCausalLM.from_pretrained(model_name).to(device).eval()

    correct_p, correct_a, correct_e, correct_logps = [], [], [], []
    incorrect_p, incorrect_a, incorrect_e, incorrect_logps = [], [], [], []
    ...
    for i in range(0, len(prompts_trimmed), batch_size):
        ...
        last = logits[torch.arange(logits.size(0), device=device), seq_ends, :]  # (B, V)

        preds = last.argmax(dim=-1)
        log_probs = last.log_softmax(dim=-1)
        batch_gold_logps = log_probs.gather(1, gold_ids.unsqueeze(1)).squeeze(1)  # (B,)
    ...
    return (
        correct_p,   correct_a,   correct_e,   correct_logps,   # <- NEW fourth item
        incorrect_p, incorrect_a, incorrect_e, incorrect_logps
    )
