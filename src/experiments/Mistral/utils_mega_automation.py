...
# ---------------------------------------------------------------------
# utils_mega_automation.py  (replace the whole run_attn_head_out_patching function)
...
def run_attn_head_out_patching(
    ...
    import math
    n_total = min(len(clean_prompts), len(corrupted_prompts), len(clean_answer_ids))
    if n_total == 0:
        print(f"↪️  Skipping {direction_label} - zero aligned prompts")
        return None

    answer_full = torch.tensor(clean_answer_ids[:n_total], device=device)
    n_layers, n_heads = tl_model.cfg.n_layers, tl_model.cfg.n_heads
    agg = None
    total_weight = 0

    # ✅ Cache only what's needed by get_act_patch_attn_head_out_all_pos
    #    TL expects per-head output = *.attn.hook_z
    def cache_filter(name: str) -> bool:
        # cover both exact and version-variant suffixes
        return name.endswith(".attn.hook_z") or name.endswith(".hook_z") \
            or name.endswith(".attn.hook_result") or name.endswith(".hook_result")

    with torch.inference_mode():
        for start in range(0, n_total, chunk_size):
            end = min(n_total, start + chunk_size)
            both = clean_prompts[start:end] + corrupted_prompts[start:end]
            tok  = tl_model.to_tokens(both)

            if max_seq_len is not None and tok.size(1) > max_seq_len:
                tok = tok[:, -max_seq_len:]

            clean_tokens     = tok[: end - start]
            corrupted_tokens = tok[end - start :]

            # Cache only the needed activations on the clean pass
            clean_logits, clean_cache = tl_model.run_with_cache(
                clean_tokens, names_filter=cache_filter
            )
            # No cache on the corrupted pass
            corrupted_logits, _ = tl_model.run_with_cache(
                corrupted_tokens, names_filter=lambda _: False
            )

            ans_chunk = answer_full[start:end]
            def logit_diff(logits):
                last = logits[:, -1, :]
                log_probs = last.log_softmax(dim=-1)
                return log_probs.gather(1, ans_chunk.unsqueeze(1)).mean()
            ...
            weight = (end - start)
            agg = (patch_chunk * weight) if agg is None else (agg + patch_chunk * weight)
            total_weight += weight

            del clean_cache, clean_logits, corrupted_logits, patch_chunk, tok, both
            torch.cuda.empty_cache()

    patch = agg / max(total_weight, 1)
    ...
    save_heatmap(
        data=patch,
        x_labels=[f"H{h}" for h in range(n_heads)],
        y_labels=[f"L{l}" for l in range(n_layers)],
        title=heatmap_title,
        filename=png_name,
    )
